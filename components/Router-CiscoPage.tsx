...
export default function RouterPage({ backendUrl }: { backendUrl: string }) {
  // CONNEXION & SECURITE
  ...
  const [interfacesNatInside, setInterfacesNatInside] = useState<string[]>(["GigabitEthernet2"]);
  ...
  const [excludeAddresses, setExcludeAddresses] = useState<string[]>(["192.168.20.1", "192.168.20.10"]);
  ...
  return (
    <div className="space-y-8">
      ...
      <footer className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex items-center justify-between shadow-2xl">
        <span className="text-xs text-slate-400"><span className="text-emerald-400 font-bold">✓ Ready</span> : Modules meurement prepares.</span>
        <button
          onClick={handleDeploy}
          disabled={isDeploying}
          className={`bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold px-8 py-3 rounded-lg text-sm transition shadow-lg cursor-pointer ${
            isDeploying ? "opacity-50 cursor-not-allowed" : ""
          }`}
        >
          {isDeploying ? "Lancement du Playbook..." : "Push Router Playbook"}
        </button>
      </footer>
