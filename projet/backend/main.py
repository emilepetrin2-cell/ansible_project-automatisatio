...
from database import get_db, UserModel, ExecutionLogModel, Base, engine

limiter = Limiter(key_func=get_remote_address)

app = FastAPI(
    title="Ansible Automation API",
    description="Backend modulaire avec authentification SQLite, Cookies HttpOnly & Worker Celery/Redis",
    version="1.3.0"
)
...
PROJECT_DIR = os.getenv("PROJECT_DIR", "/home/elpetrino/projet")
...
# =========================================================================
# GESTION ASYNCHRONE DES TACHES (CELERY + REDIS) ET BACKUPS
# =========================================================================

@app.get("/api/v1/status/{job_id}", dependencies=[Depends(allow_all_authenticated)])
async def get_job_status(job_id: str):
    """
    Recupere le statut d'une tache d'automatisation via Celery / Redis.
    """
    task_result = AsyncResult(job_id, app=celery_app)
    
    if task_result.state == "PENDING":
        return {"status": "running", "message": "Tache en attente d'execution..."}
        ...
        return {
            ...
            "message": res.get("message", "Tache terminee."),
    ...
    return {"status": task_result.state.lower(), "message": "Mise a jour du statut..."}

@app.get("/api/v1/history", dependencies=[Depends(allow_all_authenticated)])
def get_execution_history(
    ...
    """
    Recupere la liste des executions passees (Audit Log)
    """
    logs = (
...
@app.get("/api/v1/history/{log_id}", dependencies=[Depends(allow_all_authenticated)])
def get_execution_detail(log_id: int, db: Session = Depends(get_db)):
    """
    Recupere le detail complet et les logs bruts Ansible d'une tache enregistree
    """
    log = db.query(ExecutionLogModel).filter(ExecutionLogModel.id == log_id).first()
    if not log:
        raise HTTPException(status_code=404, detail="Entree d'historique introuvable.")
