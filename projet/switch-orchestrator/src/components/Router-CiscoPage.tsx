    ...
    try {
      ...
      const checkInterval = setInterval(async () => {
        ...
        try {
          ...
          if (statusData.status === "failed" || statusData.status === "FAILURE") {
            clearInterval(checkInterval);
            setIsDeploying(false);
            setExecutionError(statusData.stderr || statusData.error || statusData.message || "Echec du playbook Routeur.");
          } else if (statusData.status === "completed" || statusData.status === "SUCCESS" || statusData.status === "success") {
            clearInterval(checkInterval);
            setIsDeploying(false);
          ...
          if (attempts >= 60) {
            clearInterval(checkInterval);
            setIsDeploying(false);
            setExecutionError("Delai depasse lors du suivi de l'execution du playbook.");
          }
        } catch (e) {
          // Attente du prochain polling
        }
      }, 2000);
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
        <div className="border-b border-slate-800 pb-2 mb-4 flex items-center gap-2">
          ...
          <h2 className="text-sm font-mono uppercase text-blue-400 m-0">Playbook Modules Activation</h2>
        </div>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl grid grid-cols-1 md:grid-cols-2 gap-6">
          {triggerHardening && (
            <div className="space-y-4">
              <div className="border-b border-slate-800 pb-1 flex items-center gap-2">
                <svg className="w-4 h-4 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                <h3 className="text-xs font-mono uppercase text-cyan-400 m-0">Hardening Parameters (M2)</h3>
              </div>
          ...
          {triggerBaseServices && (
            <div className="space-y-4">
              <div className="border-b border-slate-800 pb-1 flex items-center gap-2">
                ...
                <h3 className="text-xs font-mono uppercase text-cyan-400 m-0">System Services (M3)</h3>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
      ...
      {triggerInterfaces && (
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-4">
          <div className="border-b border-slate-800 pb-2 flex items-center gap-2">
            <svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            <h2 className="text-sm font-mono uppercase text-blue-400 m-0">L3 Routing Interfaces (M4)</h2>
          </div>
                ...
                {routerInterfaces.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-900/30">
                    ...
                    <td className="p-2.5 text-right pr-4">
                      <button onClick={() => setRouterInterfaces(routerInterfaces.filter((_, i) => i !== idx))} className="text-red-400 hover:text-red-300 text-[11px] cursor-pointer inline-flex items-center gap-1">
      ...
      {triggerRouting && (
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-4">
          <div className="border-b border-slate-800 pb-2 flex items-center gap-2">
            ...
            <h2 className="text-sm font-mono uppercase text-blue-400 m-0">OSPF Protocol Configuration (M5)</h2>
          </div>
      ...
      {triggerNat && (
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-4">
          <div className="border-b border-slate-800 pb-2 flex items-center gap-2">
            ...
            <h2 className="text-sm font-mono uppercase text-blue-400 m-0">NAT & Inside/Outside Translation (M6)</h2>
          </div>
      ...
      {triggerAcls && (
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-6">
          <div className="border-b border-slate-800 pb-2 flex items-center gap-2">
            ...
            <h2 className="text-sm font-mono uppercase text-blue-400 m-0">Extended ACL Builder & Binding (M7)</h2>
          </div>
            ...
            {aces.map((ace, idx) => (
              <div key={idx} className="bg-slate-950/60 border border-slate-800 rounded-lg p-4 space-y-3 text-xs">
                <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                  ...
                  <button onClick={() => setAces(aces.filter((_, i) => i !== idx))} className="text-red-400 hover:text-red-300 text-xs cursor-pointer inline-flex items-center gap-1">
            ...
            {aclBindings.map((b, idx) => (
              <div key={idx} className="flex gap-3 items-center text-xs bg-slate-950/40 p-3 rounded-lg border border-slate-800">
                ...
                <button
                  ...
                  className="text-red-400 hover:text-red-300 text-xs cursor-pointer ml-auto flex items-center gap-1"
                >
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>
                  Remove Binding
                </button>
      ...
      {triggerDhcp && (
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-6">
          <div className="border-b border-slate-800 pb-2 flex items-center gap-2">
            ...
            <h2 className="text-sm font-mono uppercase text-blue-400 m-0">DHCP Server & DHCP Relay / Helper (M8)</h2>
          </div>
                  ...
                  <div key={idx} className="bg-slate-950/50 border border-slate-800 rounded-lg p-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs relative">
                    <div className="sm:col-span-3 flex justify-between items-center border-b border-slate-800/80 pb-2 mb-1">
                      ...
                      <button
                        ...
                        className="text-red-400 hover:text-red-300 text-[11px] font-mono bg-red-950/40 hover:bg-red-900/60 border border-red-800/50 px-2 py-0.5 rounded cursor-pointer inline-flex items-center gap-1"
                      >
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        Supprimer ce pool
                      </button>
            ...
            <div className="space-y-3 bg-slate-950/40 p-4 rounded-lg border border-cyan-900/40">
              ...
              {dhcpHelpers.map((h, i) => (
                <div key={i} className="flex gap-3 items-center text-xs">
                  ...
                  <button onClick={() => setDhcpHelpers(dhcpHelpers.filter((_, idx) => idx !== i))} className="text-red-400 text-xs cursor-pointer p-1">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>
                  </button>
                </div>
              ))}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-4xl w-full p-6 space-y-4 shadow-2xl max-h-[90vh] flex flex-col">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-sm font-mono text-cyan-400 flex items-center gap-2">
                <svg className="w-4 h-4 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2"/></svg>
                Backups (.cfg) - {hostname}
              </h3>
              <button onClick={() => { setShowBackupModal(false); setSelectedFileContent(null); }} className="text-slate-400 hover:text-slate-200">
                ...
                {backupFiles.length === 0 ? (
                  ...
                  backupFiles.map((file) => (
                    <button
                      ...
                      className="w-full text-left text-xs font-mono p-2 rounded hover:bg-slate-800 text-slate-300 transition block truncate cursor-pointer flex items-center gap-2"
                    >
        ...
        <span className="text-xs text-slate-400">
          {userRole === "auditor" ? (
            ...
            <span className="text-emerald-400 font-bold flex items-center gap-1.5">
              ...
              Ready : Session {userRole?.toUpperCase()} active
            </span>
          )}
        </span>
