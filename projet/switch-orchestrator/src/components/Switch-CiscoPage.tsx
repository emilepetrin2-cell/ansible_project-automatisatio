...
export default function SwitchPage({ backendUrl, userRole }: { backendUrl: string; userRole: string | null }) {
  // CONNEXION & SECURITE
      ...
      const checkInterval = setInterval(async () => {
        ...
        try {
          ...
          if (statusData.status === "failed" || statusData.status === "FAILURE") {
            ...
            setExecutionError(statusData.stderr || statusData.error || statusData.message || "Echec du playbook Switch.");
    ...
    <div className="space-y-8">
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
        <div className="border-b border-slate-800 pb-2 mb-4 flex items-center gap-2">
          ...
          <h2 className="text-sm font-mono uppercase tracking-wider text-blue-400 m-0">System Parameters</h2>
        </div>
                    ...
                    <tr key={v.id} className="hover:bg-slate-900/50">
                      ...
                      <td className="p-2 text-right pr-3">
                        <button
                          ...
                          className="text-red-400 hover:text-red-300 text-[10px] font-mono bg-red-950/40 hover:bg-red-900/60 border border-red-800/50 px-1.5 py-0.5 rounded cursor-pointer flex items-center gap-1 ml-auto"
                        >
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-6">
        <div className="border-b border-slate-800 pb-2 flex items-center gap-2">
          ...
          <h2 className="text-sm font-mono uppercase tracking-wider text-blue-400 m-0">Interfaces Management</h2>
        </div>
                    ...
                    <tr key={idx} className="border-b border-slate-900 hover:bg-slate-900/30">
                      ...
                      <td className="p-2 text-right pr-3">
                        <button
                          ...
                          className="text-red-400 hover:text-red-300 text-[10px] font-mono bg-red-950/40 hover:bg-red-900/60 border border-red-800/50 px-1.5 py-0.5 rounded cursor-pointer inline-flex items-center gap-1"
                        >
                          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                          Supprimer
                        </button>
              ...
              {trunkInterfaces.map((t, i) => (
                <span key={i} className="bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-md font-mono text-cyan-400 flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="text-red-400 hover:text-red-300 text-[10px] ml-1 bg-red-950/40 hover:bg-red-900/60 border border-red-800/50 p-1 rounded cursor-pointer"
                  >
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
        <div className="border-b border-slate-800 pb-2 mb-4 flex items-center gap-2">
          <svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/></svg>
          <h2 className="text-sm font-mono uppercase tracking-wider text-blue-400 m-0">SVI Management Interface</h2>
        </div>
      ...
      <footer className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
        <label className="flex items-center space-x-3 cursor-pointer select-none">
          ...
          <span className="text-sm font-medium text-slate-300">
            Execute <code className="bg-slate-950 px-1 py-0.5 rounded text-xs text-emerald-400 font-mono">write memory</code> after changes
          </span>
        </label>
        ...
        <button
          ...
          className={`w-full sm:w-auto bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-semibold px-8 py-3 rounded-lg text-sm transition-all shadow-lg ${
            isDeploying || userRole === "auditor" ? "opacity-40 cursor-not-allowed" : "cursor-pointer"
          }`}
        >
          {userRole === "auditor" ? "Action non autorisee (Auditor)" : isDeploying ? "Lancement du Playbook..." : "Push Switch Playbook"}
        </button>
      </footer>
    </div>
  );
