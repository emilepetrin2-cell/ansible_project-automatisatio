...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"provision" | "users" | "history">("provision");
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-semibold transition cursor-pointer ${
                activeTab === "history" ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-400 hover:text-slate-200"
              }`}
            >
              Historique Logs
            </button>
          ...
          <>
            ...
            {vendor !== "cisco" && (
              <div className="p-8 bg-slate-900 border border-slate-800 rounded-xl text-center text-slate-400 text-sm">
                Les playbooks Ansible pour <strong>{vendor.toUpperCase()} ({deviceType.toUpperCase()})</strong> ne sont pas encore integres.
              </div>
            )}
          </>
        )}
